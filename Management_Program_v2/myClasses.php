...
// INSERT
    function insertDB($contractDate, $custName, $phoneNumber1, $phoneNumber2, $address, $type, $product, $suits, $suitsFactory, $shirts, $shirtsFactory, $shoes, $shoesFactory, $fixDate, $price, $paid, $balance, $seller){
        $this->result = mysqli_query($this->dbConnect, 'insert into custinfo(contract_date, cust_name, main_phone_number, sub_phone_number, address, type, product, suits, suits_factory, shirts, shirts_factory, shoes, shoes_factory, fix_date, total_price, paid, balance, seller) values("'. $contractDate .'" , "'. $custName .'" , "'. $phoneNumber1 .'" , "'. $phoneNumber2 .'" , "'. $address .'" , "'. $type .'" , "'. $product .'" , "'. $suits .'" , "'. $suitsFactory .'" , "'. $shirts .'" , "'. $shirtsFactory .'" , "'. $shoes .'" , "'. $shoesFactory .'" , "'. $fixDate .'" , "'. $price .'" , "'. $paid .'", "'. $balance .'", "'. $seller .'" )') or die("Insert query error!");
        if(!($this->result)){
            $this->valid = false;
        }
    }
