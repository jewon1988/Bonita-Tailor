...
if($_SESSION['loggedin']){
    ...
    if(isset($_POST['insert'])){
        ...
        if(empty($_POST['factoryFinishDate'])){
            $factoryFinishDateErr = "공장완성일을 입력하세요..";
            $valid = false;
        }
        if(empty($_POST['settingDate'])){
            $settingDateErr = "셋팅일을 입력하세요..";
            $valid = false;
        }
        if(empty($_POST['supplyDate'])){
            $supplyDateErr = "납품일을 입력하세요..";
            $valid = false;
        }
        if(empty($_POST['shootingDate'])){
            $shootingDateErr = "촬영날짜를 입력하세요..";
            $valid = false;
        }if(empty($_POST['marriageDate'])){
            $marriageDateErr = "결혼식날짜를 입력하세요..";
            $valid = false;
        }
        ...
        if($valid){
            ...
            $connection->insertDB( $contractDate, $custName, $phoneNumber1, $phoneNumber2, $address, $type, $product, $suits, $suitsFactory, $shirts, $shoes, $fixDate, $factoryFinishDate, $settingDate, $supplyDate, $shootingDate, $marriageDate, $price, $paid, $balance);
