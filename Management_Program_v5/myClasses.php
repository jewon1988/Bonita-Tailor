...
// INSERT
    function insertDB($custNum, $contractDate, $custName, $phoneNumber1, $phoneNumber2, $type, $product, $suitsFactory, $factoryFixDate, $fixDate, $factoryFinishDate, $settingDate, $supplyDate, $shootingDate, $marriageDate, $price, $paid, $balance){
        $this->result = mysqli_query($this->dbConnect, 'insert into custinfo(cust_num, contract_date, cust_name, main_phone_number, sub_phone_number, type, product, suits_factory, factory_fix_date, fix_date, factory_finish_date, setting_date, supply_date, shooting_date, marriage_date, total_price, paid, balance) values("'. $custNum .'" , "'. $contractDate .'" , "'. $custName .'" , "'. $phoneNumber1 .'" , "'. $phoneNumber2 .'" , "'. $type .'" , "'. $product .'" , "'. $suitsFactory .'" , "'. $factoryFixDate .'" , "'. $fixDate .'" , "'. $factoryFinishDate .'" , "'. $settingDate .'" , "'. $supplyDate .'" , "'. $shootingDate .'" , "'. $marriageDate .'" , "'. $price .'" , "'. $paid .'", "'. $balance .'" )') or die("Insert query error!");
        if(!($this->result)){
            $this->valid = false;
        }
    }
...
// UPDATE
    ...
    function factoryFixDateUpdateDB($custNum, $custName, $phoneNumber1, $factoryFixDate){
        $this->result = mysqli_query($this->dbConnect, 'update custinfo set factory_fix_date = "' .$factoryFixDate. '" where cust_num = "' .$custNum. '" and cust_name = "' .$custName. '" and main_phone_number = "' .$phoneNumber1. '"') or die("Update query error!");
        if(!($this->result)){
            $this->valid = false;
        }
    }
...
// SELECT
    ...
    // by Factory Fix Date
    function selectDB7($startFactoryFixDate, $lastFactoryFixDate){
        $this->result = mysqli_query($this->dbConnect, 'select * from custinfo where factory_fix_date >= "'. $startFactoryFixDate .'" and factory_fix_date <= "'. $lastFactoryFixDate .'" order by factory_fix_date') or die("Select query error!");
        if($this->result <= 0){
            $this->valid = false;
        }
        else{
