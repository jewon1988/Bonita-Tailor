...
if($_SESSION['loggedin']){
    ...
    if(isset($_POST['select2'])){
        if(empty($_POST['startReserveDate']) || empty($_POST['lastReserveDate'])){
            $fixReserveDateErr = "조회할 예약일을 입력하세요..";
            $valid = false;
        }
        if($_POST['startReserveDate'] > $_POST['lastReserveDate']){
        ...
        if($valid){
            ...
            $startReserveDate = $_POST['startReserveDate'];
            $lastReserveDate = $_POST['lastReserveDate'];

            $connection->selectReserveDB2($startReserveDate, $lastReserveDate);
            ...
            <form method="post" action="">
                <table class="table2">
                    <tr>
                        <td>예약일 : </td>
                        <td>
                            <input type="text" name="startReserveDate" value="<?php if(!empty($_POST['startReserveDate'])) echo $_POST['startReserveDate'];  ?>"> ~
                            <input type="text" name="lastReserveDate" value="<?php if(!empty($_POST['lastReserveDate'])) echo $_POST['lastReserveDate'];  ?>"> (예 : 2017-01-01)
                        </td>
